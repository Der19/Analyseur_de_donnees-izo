                          ...
                          <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-2">
                              <input
                                type="checkbox"
                                id={`toExplain-${index}`}
                                checked={selectedVariables.includes(columnName)}
                                onChange={(e) => {
                                  e.stopPropagation()
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 max-h-64 overflow-y-auto">
                                {columnValues[columnName]
                                  ...
                                  <label key={valueIndex} className="flex items-center space-x-2 p-2 border rounded hover:bg-gray-50 cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={selectedValues[columnName]?.includes(value) || false}
                                      onChange={(e) => 
                      ...
                      <div key={`sample-${index}`} className="border border-yellow-200 rounded-lg overflow-hidden min-w-0">
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500">Colonne {index + 1}</p>
                            {selectedValues[columnName] && selectedValues[columnName].length > 0 && (
                              <p className="text-xs text-yellow-600 mt-1">✅ {selectedValues[columnName].length} valeur(s) sélectionnée(s)</p>
                            )}
                          </div>
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-2">
                              <input
                                type="checkbox"
                                checked={selectedValues[columnName] && selectedValues[columnName].length === (previewData.analysisResult?.remaining_data?.[columnName] || []).length && (previewData.analysisResult?.remaining_data?.[columnName] || []).length > 0}
                                onChange={(e) => {
                                  e.stopPropagation()
                                  const allValues = previewData.analysisResult?.remaining_data?.[columnName] || []
                                  if (e.target.checked) {
                                    setSelectedValues(prev => ({ ...prev, [columnName]: allValues }))
                                  } else {
                                    setSelectedValues(prev => {
